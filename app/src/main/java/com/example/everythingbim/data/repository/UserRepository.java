...
public class UserRepository {
     ...
     * {@code companyName} for businesses) starts with the given query string.
     *
    ...
    public LiveData<List<UserWithProfile>> searchUsersInFirestore(String query) {
        ...
        // Business documents use `companyName` instead of `username`.
        firestore.collection(COLLECTION_BUSINESSES)
                .whereGreaterThanOrEqualTo("companyName", query)
                .whereLessThanOrEqualTo("companyName", upperBound)
                .limit(20)
                .get()
                .addOnSuccessListener(snapshot -> {
     ...
     * instances. Business documents use {@code companyName} for display
